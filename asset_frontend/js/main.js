...
// Tab btn template
document.addEventListener('DOMContentLoaded', function () {
    const tab_btn_code = document.querySelectorAll('.tab-btn_code');
    const tabContent_code = document.querySelectorAll('.display_detail');

    tab_btn_code.forEach(button => {
        button.addEventListener('click', function () {
            // Remove active class from all buttons
            tab_btn_code.forEach(btn => btn.classList.remove('active'));
            
            // Add active class to the clicked button
            this.classList.add('active');
            
            // Hide all tab contents
            tabContent_code.forEach(content => content.style.display = 'none');
